...
app.post('/api/generate-futureme', async (req, res) => {
  try {
    if (!genAI) {
      // Re-read dynamically if loaded later or not set on startup
      const dynamicKey = process.env.GEMINI_API_KEY;
      if (dynamicKey) {
        genAI = new GoogleGenerativeAI(dynamicKey);
      } else {
        return res.status(500).json({
          success: false,
          error: 'Gemini API key is not configured. Please add GEMINI_API_KEY to your Netlify Environment Variables.'
        });
      }
    }
